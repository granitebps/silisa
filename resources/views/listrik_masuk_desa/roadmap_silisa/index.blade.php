    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('roadmap_silisa.index') }}" method="get">
                        ...
                        <a href="{{ route('roadmap_silisa.index') }}" class="btn btn-primary float-right">All</a>
                    </form>
                </div>
            </div>
            <br>
            ...
            <div class="card-header">
                <span>Road Map SILISA | Jumlah Data : {{$roadmap_silisa->total()}}</span>
                <a href="{{ route('roadmap_silisa.create') }}" class="btn btn-secondary float-right">Tambah</a>
            </div>
                ...
                <div class="card-body">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <?php $i = ($roadmap_silisa->currentpage()-1)* $roadmap_silisa->perpage() + 1;?>
                            @foreach ($roadmap_silisa as $row)
                                <tr>
                                    <td>{{$i++}}</td>
                                    <td>{{$row->KODE_DESA}}</td>
                                    ...
                                    <td>{{$row->NAMA_DESA}}</td>
                                    <td>
                                        <a href="{{ route('roadmap_silisa.show', ['id'=>$row->ID_DESA]) }}" class="btn btn-info">View</a>
                                    </td>
                                    <td>
                                        <a href="{{ route('roadmap_silisa.edit', ['id'=>$row->ID_DESA]) }}" class="btn btn-warning">Edit</a>
                                    </td>
                                    <td>
                                        <form id="form-id" action="{{ route('roadmap_silisa.destroy', ['id'=>$row->ID_DESA]) }}" method="post">
                                            @csrf
                                            @method('delete')
...
<script type="text/javascript">
    ...
    $('.tombol-hapus').on('click', function(e){
        ...
        Swal({
            title: 'Apakah Anda Yakin Ingin Menghapus Data Roadmap SILISA?',
            type: 'warning',
