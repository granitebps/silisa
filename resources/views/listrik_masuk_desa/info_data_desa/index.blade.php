    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('info_data_desa.index') }}" method="get">
                        ...
                        <a href="{{ route('info_data_desa.index') }}" class="btn btn-primary float-right">All</a>
                    </form>
                </div>
            </div>
            <br>
            <div class="card border-primary">
                <div class="card-header">
                    <span class="float-left">Info Data Desa | Jumlah Data : {{$info_data_desa->total()}}</span>
                    <span><a href="{{ route('info_data_desa.create') }}" class="btn btn-secondary float-right">Tambah</a></span>
                </div>
                ...
                <div class="card-body">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <?php $i = ($info_data_desa->currentpage()-1)* $info_data_desa->perpage() + 1;?>
                            @foreach ($info_data_desa as $row)
                                <tr>
                                    <td>{{$i++}}</td>
                                    <td>{{$row->KODE_DESA}}</td>
                                    ...
                                    <td>{{$row->NAMA_DESA}}</td>
                                    <td>
                                        <a href="{{ route('info_data_desa.show', ['id'=>$row->ID_DESA]) }}" class="btn btn-info">View</a>
                                    </td>
                                    <td>
                                        <a href="{{ route('info_data_desa.edit', ['id'=>$row->ID_DESA]) }}" class="btn btn-warning">Edit</a>
                                    </td>
                                    <td>
                                        <form id="form-id" action="{{ route('info_data_desa.destroy', ['id'=>$row->ID_DESA]) }}" method="post">
                                            @csrf
                                            @method('delete')
...
<script type="text/javascript">
    ...
    $('.tombol-hapus').on('click', function(e){
        ...
        Swal({
            title: 'Apakah Anda Yakin Ingin Menghapus Data Info Data Desa?',
            type: 'warning',
